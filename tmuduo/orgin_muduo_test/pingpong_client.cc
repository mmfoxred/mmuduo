...
public:
    Session(EventLoop* loop, const InetAddress& serverAddr, const string& name,
        ...
        client_.setConnectionCallback(
            bind(&Session::onConnection, this, placeholders::_1));
        client_.setMessageCallback(bind(&Session::onMessage, this,
...
public:
    Client(EventLoop* loop, const InetAddress& serverAddr, int blockSize,
        ...
        : loop_(loop),
          threadPool_(loop, "echo"),
          sessionCount_(sessionCount),
          timeout_(timeout) {
        loop->runAfter(timeout, bind(&Client::handleTimeout, this));
        if (threadCount > 1) {
