...
void EventLoop::loop() {
    ...
    while (!m_quit) {
        ...
        m_pollReturnTime = m_poller->poll(
            kPollTimeMs,
            &m_activeChannels);  //调用Epoll_wait阻塞，将活动事件返回到m_activeChannels中
        for (Channel* channel : m_activeChannels) {
            channel->handleEvent(
                m_pollReturnTime);  //显式调用Channel::handleEvent()来处理返回的事件
        }
        doPendingFunctors();
    }
...
void EventLoop::wakeUp() {
    ...
    //就会来到EventLoop::loop()中的m_poller->poll(kPollTimeMs,
    //&m_activeChannels); 然后调用EventLoop::doPendingFunctors()
    //处理queueInLoop()中上条语句加入的回调事件 m_pendingFunctors.push_back(cb);
